                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                                ...
                                <tbody>
                                    <?php if(is_array($info)): foreach($info as $key=>$value): ?><tr class="gradeX">
	                                        ...
	                                        <td><img src="/watch_shop/Public/admin/Uploads/Products/<?php echo ($value["picurl"]); ?>" width='78' height='59' style='border:1px #eee solid'/></td>
	                                        <td class="center"><?php echo ($value["stock"]); ?></td>
	                                        <td class="center"><?php echo (date("Y-m-d H:i:s",$value["addtime"])); ?></td>
	                                        <?php if(($value["offsale"]) == "1"): ?><td>上架</td><?php endif; ?>
                                            <?php if(($value["offsale"]) == "0"): ?><td>下架</td><?php endif; ?>
                                                          
	                                        <td class="center"><?php echo ($value["price"]); ?></td>
                                            <td>
                                                <form action="/watch_shop/index.php/Admin/Goodstate/modify" method="POST" target="_self">
                                                    <?php if(($value["offsale"]) == "0"): ?><input type="hidden" name="offsale" value="1" />
                                                        <input type="hidden" name="pid" value="<?php echo ($value["pid"]); ?>" /><?php endif; ?>
                                                    <?php if(($value["offsale"]) == "1"): ?><input type="hidden" name="offsale" value="0" />
                                                        <input type="hidden" name="pid" value="<?php echo ($value["pid"]); ?>" /><?php endif; ?>
                                                    <?php if(($value["offsale"]) == "0"): ?><button>上架</button><?php endif; ?>
                                                    <?php if(($value["offsale"]) == "1"): ?><button>下架</button><?php endif; ?>
                                                </form>
                                            </td>
